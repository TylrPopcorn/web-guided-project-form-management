...
export default function App() {
  ...
  const submitForm = () => {
    ...
    const checkUser = formValues.username.split(" ");

    if (!newFriend.username || checkUser.length > 1) {
      setFormErrors("Gotta enter a username ya chump AND NO SPACES YA JOKER COME OONNNNNN!");
      return;
    }
    // C. -> Create -> POST
    // R. -> Read   -> GET
